...
namespace pocketmine {
	...
	if(php_sapi_name() !== "cli"){
		$logger->critical("You must run CatCore using the CLI.");
		++$errors;
	}
	...
	if(extension_loaded("pocketmine")){
		if(version_compare(phpversion("pocketmine"), "0.0.1") < 0){
			$logger->critical("You have the native CatCore extension, but your version is lower than 0.0.1.");
			++$errors;
		}elseif(version_compare(phpversion("pocketmine"), "0.0.4") > 0){
			$logger->critical("You have the native CatCore extension, but your version is higher than 0.0.4.");
			++$errors;
		}
	}

	if(extension_loaded("xdebug")){
		$logger->warning("You are running CatCore with Xdebug enabled. This has a major impact on performance.");
	}
	...
	$logger->info("§8[§l§6Cat§eCore§r§8] §fОстановлены все функции сервера!");
